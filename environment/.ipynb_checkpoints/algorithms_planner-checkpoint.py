...
class Planner:
    ...
    def value_iteration(self, gamma=1.0, n_iters=1000, theta=1e-10):
        ...
        #converge_id = 0
        curr_n_iters = n_iters
        while i < curr_n_iters-1 and not converged:
            i += 1
            ...
            V_track[i] = V
            Q_track[i] = Q
            pi_track.append({s__:a__ for s__, a__ in enumerate(np.argmax(Q, axis=1))})
        
            if (not converged) and (i == curr_n_iters-2):

                if curr_n_iters < 10*n_iters:
                    
                    
                    V_track = np.append(V_track, np.zeros((n_iters, len(self.P)), dtype=np.float64) ,axis =0 )
                    Q_track = np.append(Q_track, np.zeros((n_iters,len(self.P), len(self.P[0])), dtype=np.float64),axis =0 )
                    
                    
                    curr_n_iters+=n_iters
        
        if not converged:
            warnings.warn("Max iterations reached before convergence.  Check theta and n_iters.  "+str(i))
        
        pi = {s:a for s, a in enumerate(np.argmax(Q, axis=1))}
        
        #pi_track = [{s:a for s, a in enumerate(np.argmax(Q_i, axis=1))} for Q_i in 
                            
                    
        #            Q_track]
        
        return V, V_track[:i+1], pi,pi_track ,i
