...
class RL:
    ...
    @staticmethod
    def find_convergence(Q_track,window=100,threshold = 0.002):

        
        
        #print(len(Q_track))
        
        #lis_2 = np.max(Q_track[-window:] - Q_track[-window-1:-1])
        lis_2 = np.max(np.abs(Q_track[1:] - Q_track[:-1]))
        

        #lis = np.array([ np.abs(i.max()) for i in  (Q_track - np.roll(Q_track, shift =1,axis =0))]
        #        )
        #lis_2 = RL.mov_avg(lis, w=100, axis = 0)
        ...
        #print(lis_2,np.argmax(np.array(lis_2)<=threshold))#max(np.array(lis_2)))

        return lis_2<threshold#np.argmax(np.array(lis_2)<=threshold)
    ...
    def q_learning(self,
        ...
        if tqdm_:
            rang = n_episodes#tqdm(range(n_episodes), leave=False)
        else:
            rang = n_episodes#range(n_episodes)
        ...
        converged = 0
        while (e<rang_2-1) and (not converged) :
        #for e in rang:
            ...
            if (e>window) and (e%(window*10) ==1):
                #print(e,window,len(Q_track),Q_track[e-window:e+1])
                converged = RL.find_convergence(Q_track[e-window:e+1],window=window,threshold = threshold)
                if converged:
                    converged = e
                #converged 
            ...
            if (e == rang_2-2) and (not converged):
                if rang_2 <=max_mul*rang:
                    #converged = RL.find_convergence(Q_track[:e+1],window=window,threshold = threshold)
                    
                    if not converged:
                    
                        rang_2+=rang
                        Q_track = np.append(Q_track, np.zeros((n_episodes, nS, nA), dtype=np.float64),axis =0 )
                        alphas = np.append(alphas,np.ones(len(alphas))*min_alpha)
                        epsilons = np.append(epsilons,np.ones(len(epsilons))*min_epsilon)
                        
                        #RL.decay_schedule(init_alpha,
                        #            min_alpha,
                        #            alpha_decay_ratio,
                        #                        rang_2)
                        #epsilons = RL.decay_schedule(init_epsilon,
                    ...
                    converged = e
                    warnings.warn("Max iterations reached before convergence.  Check theta and n_iters.  "+str(e))
            e+=1
        ...
        Q_track = Q_track[:converged]
        
        return Q, V, pi, Q_track, pi_track, converged
